...
class PluginContext:
    ...
    arguments that were used to launch ``Helios``. It also provides access to APIs
...
class BasePlugin(ABC):
    ...
    def logger(self) -> logging.Logger:
        return logging.getLogger('helios.extensibility.plugin.BasePlugin#{0}'.format(self.name))

    def set_context(self, context: PluginContext) -> None:
        """
        Set the :class:`~helios.extensibility.plugin.PluginContext` for this plugin.
        """
        self.context = context

    def configure_parser(self, arg_parser: ArgumentParser, subparser: _SubParsersAction) -> None:
        """
        Called at startup, giving the plugin a chance to amend the Helios CLI argument parser
        """
        pass
    ...
    def handle_event(self, activation_event: BaseEvent) -> None:
        ...
        before :meth:`~helios.extensibility.plugin.BasePlugin.should_start` is called
        """
...
class BaseMainProcessPlugin(BasePlugin):
    """
    A ``BaseMainProcessPlugin`` overtakes the whole main process before most of the Helios boot
    process had a chance to start. In that sense it redefines the whole meaning of the ``helios``
    process.
    """
    pass
