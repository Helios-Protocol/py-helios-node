...
from hp2p.cancellable import CancellableMixin
...
class PeerRequestHandler(CancellableMixin):
    ...
    async def handle_get_block_bodies(self, peer: HLSPeer, block_hashes: List[Hash32]) -> None:
    ...
    async def handle_get_receipts(self, peer: HLSPeer, block_hashes: List[Hash32]) -> None:
        if not peer.is_operational:
            return
    ...
    async def handle_get_node_data(self, peer: HLSPeer, node_hashes: List[Hash32]) -> None:
        if not peer.is_operational:
            return
