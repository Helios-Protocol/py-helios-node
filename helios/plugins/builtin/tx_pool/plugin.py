...
from helios.plugins.builtin.tx_pool.pool import (
    TxPool,
)
from helios.plugins.builtin.tx_pool.validators import (
...
class TxPlugin(BaseAsyncStopPlugin):
    ...
    def handle_event(self, activation_event: BaseEvent) -> None:
        if isinstance(activation_event, HeliosStartupEvent):
        ...
        if isinstance(activation_event, ResourceAvailableEvent):
            if activation_event.resource_type is HLSPeerPool:
