...
from hvm.tools.logging import TraceLogger
...
from helios.protocol.hls import (
    constants as eth_constants,
)
from helios.sync.full.hexary_trie import (
    HexaryTrieSync,
    SyncRequest,
)
...
class StateDownloader(BaseService, PeerSubscriber):
    ...
    def __init__(self,
        ...
        self._peer_missing_nodes: Dict[HLSPeer, Set[Hash32]] = collections.defaultdict(set)
    ...
    def deregister_peer(self, peer: BasePeer) -> None:
        ...
        self._peer_missing_nodes.pop(cast(HLSPeer, peer), None)

    async def get_peer_for_request(self, node_keys: Set[Hash32]) -> HLSPeer:
    ...
    async def _handle_msg(
            self, peer: HLSPeer, cmd: Command, msg: _DecodedMsgType) -> None:
    ...
    async def _handle_get_block_headers(self, peer: HLSPeer, request: HeaderRequest) -> None:
    ...
    async def _request_and_process_nodes(self, peer: HLSPeer, batch: Tuple[Hash32, ...]) -> None:
...
class TrieNodeRequestTracker:
    ...
    def __init__(self, reply_timeout: int, logger: TraceLogger) -> None:
        ...
        self.logger = logger
        self.active_requests: Dict[HLSPeer, Tuple[float, Tuple[Hash32, ...]]] = {}
