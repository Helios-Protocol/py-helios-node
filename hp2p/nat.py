...
class UPnPService(BaseService):
    ...
    def __init__(self, port: int, rpc_port: int, token: CancelToken = None) -> None:
        """
    ...
    def current_mapping(self) -> PortMapping:
        if self._mapping is None:
            unbound = ':%d, :%d' % (self.port, self.rpc_port)
    ...
    async def _add_nat_portmap(self, upnp_dev: upnpclient.upnp.Device) -> str:
        ...
        self._mapping = PortMapping(
            '%s:%d, %s:%d' % (internal_ip, self.port, internal_ip, self.rpc_port),
            '%s:%d, %s:%d' % (external_ip, self.port, external_ip, self.rpc_port)
        )
