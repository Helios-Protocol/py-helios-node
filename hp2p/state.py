...
from hp2p.exceptions import OperationCancelled
from hp2p.peer import BasePeer, ETHPeer, PeerPool, PeerPoolSubscriber
from hp2p.service import BaseService
from hp2p.utils import get_process_pool_executor


class StateDownloader(BaseService, PeerPoolSubscriber):
    logger = logging.getLogger("hp2p.state.StateDownloader")
...
class StateSync(HexaryTrieSync):
    ...
    def __init__(self, root_hash, db):
        super().__init__(root_hash, db, logging.getLogger("hp2p.state.StateSync"))
...
def _test():
    ...
    logging.getLogger('hp2p.state.StateDownloader').setLevel(log_level)
