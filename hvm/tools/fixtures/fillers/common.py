...
from hvm.tools._utils.vyper import (
    compile_vyper_lll,
)
...
def expect(post_state=None, networks=None, transaction=None):
    ...
    is played by :func:`~hvm.tools.fixtures.fillers.execution`).

    * ``post_state`` is a list of state definition in the same form as expected
      by :func:`~hvm.tools.fixtures.fillers.pre_state`. State items that are
