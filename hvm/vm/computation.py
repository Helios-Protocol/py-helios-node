...
from hvm.vm.logic.invalid import (
    InvalidOpcode,
)
...
from hvm.vm.opcode import (  # noqa: F401
    Opcode
)
...
class BaseComputation(Configurable, metaclass=ABCMeta):
    ...
    The base class for all execution computations.
      ...
      .. note::
        ...
        Each :class:`~hvm.vm.computation.BaseComputation` class must be configured with:
    ...
    def consume_gas(self, amount: int, reason: str) -> None:
        ...
        Raise `hvm.exceptions.OutOfGas` if there is not enough gas remaining.
        """
