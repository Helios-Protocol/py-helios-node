...
async def test_fast_sync_1(request, event_loop):
    ...
    node_2.min_gas_db.initialize_historical_minimum_gas_price_at_genesis(min_gas_price=1, net_tpc_cap=100)
...
async def test_fast_sync_4(request, event_loop):
    ...
    client_node.min_gas_db.initialize_historical_minimum_gas_price_at_genesis(min_gas_price=1, net_tpc_cap=100)
...
async def test_additive_sync_2(request, event_loop):
    ...
    node_1 = TestnetChain(server_db, TESTNET_GENESIS_PRIVATE_KEY.public_key.to_canonical_address())
    node_1.min_gas_db.initialize_historical_minimum_gas_price_at_genesis(min_gas_price = 1, net_tpc_cap=100)
    await _test_sync_with_variable_sync_parameters(request, event_loop, client_db, server_db, ensure_blockchain_databases_identical, ADDITIVE_SYNC_STAGE_ID)
...
async def test_additive_sync_4(request, event_loop):
    ...
    server_node.min_gas_db.initialize_historical_minimum_gas_price_at_genesis(min_gas_price=1, net_tpc_cap=100)
...
async def _setup_test_import_blocks(request,
    ...
    node_1 = TestnetChain(server_db, TESTNET_GENESIS_PRIVATE_KEY.public_key.to_canonical_address())
    node_1.min_gas_db.initialize_historical_minimum_gas_price_at_genesis(min_gas_price=node_min_gas_price, net_tpc_cap=100)
    node_2 = TestnetChain(client_db, TESTNET_GENESIS_PRIVATE_KEY.public_key.to_canonical_address())
    node_2.min_gas_db.initialize_historical_minimum_gas_price_at_genesis(min_gas_price=node_min_gas_price, net_tpc_cap=100)
